...
const Fetch = async () => {
    try {
        const res = await OrderService.getSpecific(props.id);
        if (res.data) {
...
<template> 
    ...
    <section v-else class="py-24 relative">
        <div class="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto">
            ...
            <h6 class="font-medium text-xl leading-8 text-black mb-3">Hello, {{" " + order?.firstName}}</h6>
            <p class="font-normal text-lg leading-8 text-gray-500 mb-11">Your delivery will be delivered to you shortly</p>
            <div
                ...
                <div class="box group">
                    ...
                    <h6 class="font-semibold font-manrope text-2xl leading-9 text-black">{{new Date(order!.orderDate)}}</h6>
                </div>
                <div class="box group">
                ...
                <div class="box group">
                    ...
                    <h6 class="font-semibold font-manrope text-2xl leading-9 text-black">{{ order!.addressLine }}
                    </h6>
                </div>
            </div>

            <div v-for="item in order!.orderItemsCollection" :key="item.id" class="grid grid-cols-7 w-full pb-6 border-b border-gray-100">
                <div class="col-span-7 min-[500px]:col-span-2 md:col-span-1">
                    <img src="https://images.pexels.com/photos/13728043/pexels-photo-13728043.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" alt="Skin Care Kit image" class="w-full">
                </div>
                <div
                    ...
                    <div class="flex flex-col min-[500px]:flex-row min-[500px]:items-center justify-between">
                        <div class="">
                            <h5 v-if="item.keyboard" class="font-manrope font-semibold text-2xl leading-9 text-black mb-6">{{ item.keyboard.name }}
                            </h5>
                            <h5 v-else class="font-manrope font-semibold text-2xl leading-9 text-black mb-6">{{ item.part?.partName }}
                            </h5>
            ...
            <div class="flex items-center justify-center sm:justify-end w-full my-6">
                <div class=" w-full">
                    <div class="flex items-center justify-between py-6 border-y border-gray-100">
                        ...
                        <p class="font-manrope font-bold text-2xl leading-9 text-indigo-600">${{ calculateTotalCost(order!) }}</p>
                    </div>
                </div>
            </div>
            <div class="data ">
                ...
                <h6 class="font-manrope font-bold text-2xl leading-9 text-black mb-3">Thank you for shopping
                    with us!</h6>
                <p class="font-medium text-xl leading-8 text-indigo-600">Team KeyboardFans</p>
            </div>
        </div>
    </section>                                 
</template>