...
const storedBuildId = useBuildStore()
let keyboardBuild = ref<IkeyboardBuildInfo>()

const fetch = async () => {
    try {
        const res = await KeyboardBuildService.getSpecific(storedBuildId.buildId!)

        if (res.data) {
            keyboardBuild.value = res.data
        }
    } catch (error) {
    ...
    <div v-else class="overflow-x-auto min-w-full">
        ...
        <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
            ...
            <tbody class="divide-y divide-gray-200">
                <tr v-for="partInBuild in keyboardBuild?.buildParts" :key="partInBuild.id">
