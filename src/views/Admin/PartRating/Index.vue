...
let PartRatings = ref<IPartRatingInfo[]>([])
let filterParam = ref("all");
let searchQuery = ref("");

const fetchPartRatings = async () => {
  try {
    const response = await PartRatingService.getAll()

    if (response.data) {
      PartRatings.value = response.data
    }
  } catch (error) {
    console.error('Error fetching part ratings:', error)
  } finally {
...
const filteredRatings = computed( () => {
  switch (filterParam.value) {
    case ("user"):
      return PartRatings.value.filter(rating => 
          rating.user?.email.toLowerCase().includes(searchQuery.value.toLowerCase()))
    case ("part"):
      return PartRatings.value.filter(rating => 
          rating.part?.partName.toLowerCase().includes(searchQuery.value.toLowerCase()))
    case ("comment"):
      return PartRatings.value.filter(rating => 
          rating.comment.toLowerCase().includes(searchQuery.value.toLowerCase()))
    case ("rating"):
    return PartRatings.value.filter(rating => 
      rating.rating.toString().toLowerCase().includes(searchQuery.value.toLowerCase())
        )
    default:
      return PartRatings.value.filter(rating => 
          rating.user?.email.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
  ...
  <div v-else class="overflow-x-auto min-w-full">
    ...
    <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
      ...
      <tbody class="divide-y divide-gray-200">
        <tr v-for="partRating in filteredRatings" :key="partRating.id">
          <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ partRating.user.email }}</td>
          <td class="whitespace-nowrap px-4 py-2 text-gray-700">
            {{ partRating.part!.partName }}
          </td>
          <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ partRating.comment }}</td>
          <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ partRating.rating }}</td>
          <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ partRating.postedAt }}</td>
          <td class="whitespace-nowrap px-4 py-2 text-right">
            <div class="inline-flex rounded-lg border border-gray-100 bg-gray-100 p-1">
              <router-link
                :to="`Edit/${partRating.id}`"
