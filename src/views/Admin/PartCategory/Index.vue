...
let PartCategories = ref<IPartCategoryInfo[]>([])
...
const filteredPartCategories = computed( () => {
  switch (filterParam.value) {      
    case ("category"):
      return PartCategories.value.filter(partCategory => 
        partCategory.category?.categoryName.toLowerCase().includes(searchQuery.value.toLowerCase())
      )
    case ("keyboard"):
      return PartCategories.value.filter(partCategory => 
      partCategory.keyboard?.name.toLowerCase().includes(searchQuery.value.toLowerCase())
    )
    case ("part"):
    return PartCategories.value.filter(partCategory => 
      partCategory.part?.partName.toLowerCase().includes(searchQuery.value.toLowerCase()))
    default:
    return PartCategories.value.filter(partCategory => 
        partCategory.category?.categoryName.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
        partCategory.keyboard?.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
        partCategory.part?.partName.toLocaleLowerCase().includes(searchQuery.value.toLowerCase())
      )
    }
})
  ...
  <div v-else class="overflow-x-auto min-w-full">
    ...
    <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
      ...
      <tbody class="divide-y divide-gray-200">
        <tr v-for="partCategory in filteredPartCategories" :key="partCategory.id">
          <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ partCategory.category!.categoryName }}</td>
          <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ partCategory.part?.partName }}</td>
          <td class="whitespace-nowrap px-4 py-2 text-gray-700">
            {{ partCategory.keyboard?.name }}
          </td>
          <td class="whitespace-nowrap px-4 py-2 text-right">
            <div class="inline-flex rounded-lg border border-gray-100 bg-gray-100 p-1">
              <router-link
                :to="`Edit/${partCategory.id}`"
