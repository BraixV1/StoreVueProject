...
let KeyboardBuilds = ref<IkeyboardBuildInfo[]>([])
let SearchQuery = ref("");
let filterParam = ref ("all");

const fetchKeyboards = async () => {
  try {
    const response = await KeyboardBuildService.getAll()

    if (response.data) {
      KeyboardBuilds.value = response.data
    }
  } catch (error) {
...
const FilteredBuilds = computed(() => {
  switch (filterParam.value){
    case ("name"):
      return KeyboardBuilds.value.filter(build =>
      build.buildName.toLowerCase().includes(SearchQuery.value.toLowerCase()))
    case ("desc"):
      return KeyboardBuilds.value.filter(build => 
      build.description.toLowerCase().includes(SearchQuery.value.toLowerCase()))
    case ("user"):
      return KeyboardBuilds.value.filter(build => 
      build.appUser?.email.toLowerCase().includes(SearchQuery.value.toLowerCase())
      )
    default:
    return KeyboardBuilds.value.filter(build => 
      build.buildName.toLowerCase().includes(SearchQuery.value.toLowerCase())
...
<template>
  ...
  <div v-else class="overflow-x-auto min-w-full">
    ...
    >
      ...
      <span class="text-sm font-medium transition-all group-hover:ms-4"> Add new Keyboard Build </span>
    </router-link>
    <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
      <thead class="text-left">
        <tr>
          <th class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">Keyboard build Name</th>
