...
const filteredKeyboards = computed(() => {
    switch (filterParam.value) {
        case 'name':
            return keyboards.value.filter((keyboardItem) =>
                keyboardItem.name.toLowerCase().includes(searchQuery.value.toLowerCase())
            )
        case 'desc':
            return keyboards.value.filter((keyboardItem) =>
                keyboardItem.description.toLowerCase().includes(searchQuery.value)
            )
        case 'code':
            return keyboards.value.filter((keyboardItem) =>
                keyboardItem.itemCode.toLowerCase().includes(searchQuery.value.toLowerCase())
            )
        case 'Price':
            return keyboards.value.filter((keyboardItem) =>
                keyboardItem.price.toString().includes(searchQuery.value)
            )
        default:
            return keyboards.value.filter(
                (keyboardItem) =>
                    keyboardItem.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
                    keyboardItem.itemCode.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
                    keyboardItem.description.toLowerCase().includes(searchQuery.value) ||
                    keyboardItem.price.toString() == searchQuery.value
            )
    }
})
</script>
    ...
    <div v-else class="overflow-x-auto min-w-full">
        <div class="pb-2 flex flex-col md:flex-row gap-3">
            ...
            >
                ...
                <option value="name">Keyboard name</option>
        ...
        <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
            <thead class="text-left">
                <tr>
                    ...
                    <th class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">ItemCode</th>
                    <th class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">Price</th>
                    <th class="px-4 py-2"></th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-200">
                <tr v-for="keyboard in filteredKeyboards" :key="keyboard.id">
                    ...
                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ keyboard.price }}</td>
