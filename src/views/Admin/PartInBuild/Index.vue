...
let buildParts = ref<IPartInBuildInfo[]>([])
let filterParam = ref('all')
let searchQuery = ref('')

const fetchKeyboards = async () => {
    try {
        const response = await PartInBuildService.getAll()

        if (response.data) {
            buildParts.value = response.data
        }
    } catch (error) {
...
const filteredbuildParts = computed(() => {
    switch (filterParam.value) {
        case 'build':
            return buildParts.value.filter((buildItem) =>
                buildItem.keyboardBuild?.buildName
                    .toLowerCase()
                    .includes(searchQuery.value.toLowerCase())
            )
        case 'part':
            return buildParts.value.filter((buildItem) =>
                buildItem.part?.partName.toLowerCase().includes(searchQuery.value.toLowerCase())
            )
        default:
            return buildParts.value.filter(
                (buildItem) =>
                    buildItem.part?.partName
                        .toLowerCase()
                        .includes(searchQuery.value.toLowerCase()) ||
                    buildItem.keyboardBuild?.buildName
                        .toLowerCase()
    ...
    <div v-else class="overflow-x-auto min-w-full">
        <div class="pb-2 flex flex-col md:flex-row gap-3">
            ...
            >
                ...
                <option value="build">keyboard build</option>
        ...
        >
            ...
            <span class="text-sm font-medium transition-all group-hover:ms-4">
                Add new part to keyboard build</span
            >
        </router-link>
        <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
            ...
            <tbody class="divide-y divide-gray-200">
                <tr v-for="partInBuild in filteredbuildParts" :key="partInBuild.id">
                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                        {{ partInBuild.keyboardBuild.buildName }}
                    </td>
                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                        {{ partInBuild.part!.partName }}
                    </td>
                    <td class="whitespace-nowrap px-4 py-2 text-right">
                        <div class="inline-flex rounded-lg border border-gray-100 bg-gray-100 p-1">
                            <router-link
                                :to="`Edit/${partInBuild.id}`"
