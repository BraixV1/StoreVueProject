...
let WarehouseParts = ref<IWarehousePartInfo[]>([])
let searchQuery = ref('')
let filterParam = ref('all')

const fetch = async () => {
    try {
        const response = await WarehousePartService.getAll()

        if (response.data) {
            WarehouseParts.value = response.data
        }
    } catch (error) {
...
fetch()
const filteredWarehousePart = computed(() => {
    switch (filterParam.value) {
        case 'keyboard':
            return WarehouseParts.value.filter((whPart) =>
                whPart.keyboard?.name.toLowerCase().includes(searchQuery.value.toLowerCase())
            )
        case 'part':
            return WarehouseParts.value.filter((whPart) =>
                whPart.part?.partName.toLowerCase().includes(searchQuery.value.toLowerCase())
            )
        case 'warehouse':
            return WarehouseParts.value.filter((whPart) =>
                whPart.warehouse?.warehouseName
                    .toLowerCase()
                    .includes(searchQuery.value.toLowerCase())
            )
        default:
            return WarehouseParts.value.filter(
                (whPart) =>
                    whPart.keyboard?.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
                    whPart.part?.partName.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
                    whPart.warehouse?.warehouseName
    ...
    <div v-else class="overflow-x-auto min-w-full">
        ...
        <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
            ...
            <tbody class="divide-y divide-gray-200">
                <tr v-for="warehousePart in filteredWarehousePart" :key="warehousePart.id">
                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                        {{ warehousePart.warehouse!.warehouseName }}
                    </td>
                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                        {{ warehousePart.part?.partName }}
                    </td>
                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                        {{ warehousePart.keyboard?.name }}
                    </td>
                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
