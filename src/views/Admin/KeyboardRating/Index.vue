...
const fetchKeyboards = async () => {
  try {
    const response = await KeyboardRatingService.getAll()

    if (response.data) {
      keyboardRatings.value = response.data
    }
  } catch (error) {
    console.error('Error fetching keyboard ratings:', error)
  } finally {
...
const filteredRatings = computed( () => {
  switch(filterParam.value){
    case ("all"):
      return keyboardRatings.value.filter(rating =>
        rating.keyboard?.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
        ...
        rating.rating.toString().includes(searchQuery.value.toLowerCase())
      )
    case ("user"):
      return keyboardRatings.value.filter(rating =>
        rating.user?.email.toLowerCase().includes(searchQuery.value.toLowerCase())
  ...
  <div v-else class="overflow-x-auto min-w-full">
    <div class="pb-2 flex flex-col md:flex-row gap-3">
      <div class="flex">
        ...
        />
        <button class="bg-indigo-500 text-white rounded-r px-2 md:px-3 py-0 md:py-1">
          Search
        </button>
      </div>
      <select
      ...
      >
        ...
        <option value="kbname">keyboard name</option>
    ...
    <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
      ...
      <tbody class="divide-y divide-gray-200">
        <tr v-for="keyboardRating in filteredRatings" :key="keyboardRating.id">
          <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ keyboardRating.user.email }}</td>
