...
export const useAuthStore = defineStore('auth', () => {
  ...
  watch(
    [jwt, refreshToken, firstName, lastName, roles, email],
    ([newJwt, newRefreshToken, newFirstName, newLastName, newRoles, newEmail]) => {
  ...
  function updateAuthData(newJwt: string, newRefreshToken: string, newUserData: { firstName?: string; lastName?: string; roles?: string[], email?: string }) {
    jwt.value = newJwt
  ...
  return { jwt, refreshToken, firstName, lastName, roles, email, isAuthenticated, isAdmin, updateAuthData, clearAuthData }
})
